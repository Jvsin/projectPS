...
class Server:
    def __init__(self, server_id, host, port):
        ...
        print(f'Serwer {self.server_id} opened on {self.host}:{self.port}')
    ...
    def client_handler(self, client_socket):
        try:
            ...
            print(f'Client handler: Socket error: {client_socket}')
        finally:
    ...
    def handle_message(self, message, client_socket):
        try:
            ...
            print(f'Validate: no such element in message {e}')
    ...
    def handle_register(self, message_data, client_socket):
        ...
        if mode == 'producer':
            if topic not in topics:
                ...
                print(f'Register: Topic {topic} is already exist')
            ...
            if client_id not in topics[topic]['producers']:
                ...
                print(f'Register: Registered producer {client_id} for topic {topic}')
            else:
                print(f'Register: Topic {topic} is already exist')
            ...
            if client_socket not in topics[topic]['subscribers']:
                ...
                print(f'Register: Registered subscriber for {topic}')
            else:
                print(f'Register: Client {clients[client_socket]} is not a subscriber of {topic}')
        else:
            print(f'Register: unsupported mode {mode}')
        ...
        if topic in topics:
            if mode == 'producer':
                if client_id in topics[topic]['producers'] and topics[topic]['producers'][client_id] == client_socket:
                    ...
                    print(f'Withdraw: Deleted {topic}')
                else:
                    print(f'Withdraw: {client_id} is not a producer of {topic}')
            elif mode == 'subscriber':
                if client_id in topics[topic]['subscribers']:
                    # topics[topic]['subscribers'].remove(client_socket)
                    del topics[topic]['subscribers'][client_id]
                    print(f'Withdraw: Deleted subscriber of {topic}')
                else:
                    print(f'Withdraw: Client {clients[client_socket]} is not a subscriber of {topic}')
            else:
                print(f'Withdraw: No supported type {mode}')
        else:
            print(f'Withdraw: Topic {topic} does not exist')
        ...
        if topic in topics:
            if topics[topic]['subscribers']:
                for subscriber_socket in topics[topic]['subscribers'].values():
                    ...
                    print(f'KKW: Added new message to {topic}')
            else:
                print(f'Topics: No subscribers of topic: {topic}')
        else:
            print(f'Topics: Topic {topic} does not exist')
    ...
    def handle_status(self, message_data, client_socket):
        ...
        print(KKW)
        print(f'KKW: Added new message for {clients[client_socket]}')
    ...
    def monitoring_thread(self):
        while True:
            ...
            if KKO:
                message = KKO.pop(0)
                print(f'KKO: Message taken: {message}')
            ...
            if KKW:
                ...
                try:
                    ...
                    print(f'KKW: Message send to {clients[client_socket]}: {message}')
                except socket.error as e:
                    print(f'KKW: Error while message sending: {e}')
        ...
        try:
            ...
            for field in required_fields:
                if field not in message_data:
                    print(f'Validate: Incorrect message format: {field}')
                    return False
    ...
    def manage_message(self, message):
        ...
        if message_type == 'register':
            ...
            print(f'MessageManager: unsupported message type: {message_type}')

    def user_interface_thread(self):
        while True:
            time.sleep(3)
            command = input("Write command (np. 'show topics', 'show clients'): ")
...
if __name__ == "__main__":
    ...
    server = Server(server_id, host, port)
    server.start()